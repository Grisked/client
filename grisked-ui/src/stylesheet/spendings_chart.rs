...
const OUT_FILE_NAME: &str = "assets/pie-chart.svg";

pub fn draw(infos: &[(Option<Label>, f64)]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let radius = 300.0;
    let sizes: Vec<f64> = infos.iter().map(|(_, value)| *value).collect();
